...
def createdataset(matpath, datapath, ratio, patch, strides, mode, crop_size=None, channel_first=True, key='rad',
    ...
    if not os.path.exists(datapath):
        ...
        if not os.path.exists(train_dir) or not os.path.exists(test_dir):
            ...
            for mat_file in mat_list:
                ...
                #print(f"加载数据形状: {data.shape}, 数据范围: 最小值 {data.min()}，最大值 {data.max()}")
    ...
    print('{} images for training, {} images for testing.'.format(len(glob.glob(train_dir + f'*{ext}')), len(glob.glob(test_dir + f'*{ext}'))))
...
def makedataloader(matpath, datapath, patch, strides, mode, channel_first, key, loader, batch_size=(64, 64), blind='no',
    ...
    # 打印数据集路径及文件数量
    print(f"训练集路径: {datapath + 'train/'}, 测试集路径: {datapath + 'test/'}")
    print(f"训练集图像数量: {len(glob.glob(datapath + 'train/*.mat'))}, 测试集图像数量: {len(glob.glob(datapath + 'test/*.mat'))}")
    ...
    testdataset = ICVLData(datapath + 'test/', len(glob.glob(datapath + 'test/*.mat')), transforms=transform_test,
                           train=False)
    # 打印处理前的训练和测试数据集的数量
    print(f"训练集样本数量: {len(traindataset)}, 测试集样本数量: {len(testdataset)}")

    trainDataloader = DataLoader(dataset=traindataset, batch_size=batch_size[0], shuffle=True, num_workers=2)#10
    testDataloader = DataLoader(dataset=testdataset, batch_size=batch_size[1], shuffle=False, num_workers=2)#3
