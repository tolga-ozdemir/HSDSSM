...
class Block_mamba(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("Block_mamba:",x.shape)
        x = x + self.drop_path(self.attn(x))
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W))#self.norm2(x)
        return x
...
class UpSample(nn.Module):
    def __init__(self, input_resolution, in_channels, scale_factor):
        ...
        if self.factor == 2:
            ...
            self.up_p = nn.Sequential(nn.Conv2d(in_channels, 2 * in_channels, 1, 1, 0, bias=False),#16
                                      nn.PReLU(),
                                      nn.PixelShuffle(2),#scale_factor
                                      nn.Conv2d(in_channels//2, in_channels//2, 1, stride=1, padding=0, bias=False))
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth,norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False, mlp_ratio = 4., drop_path=0.):
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, upsample=None, use_checkpoint=False,mlp_ratio = 4., drop_path=0.,norm_layer=nn.LayerNorm,):
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            x = self.norm(x)
        # print("PatchEmbed output:",x.shape)
...
class MUNet(nn.Module):
    ...
    def __init__(self, img_size=None, patch_size=4, in_chans=3, out_chans=3,
                 embed_dim=96, depths=[2, 2, 2, 2],  mlp_ratio=4.,
    ...
    def forward(self, x):
        if x.dim() == 5 and x.size(1) == 1:
            x = x.squeeze(1)  # [B, 31, H, W]
        input_x = x  # Save for residual connection
        ...
        x = self.up_x4(x)
        out = self.output(x)
        out = out + input_x  # Residual connection
        return out
...
if __name__ == '__main__':
    ...
    height = 128
    width = 128
    x = torch.randn((1, 31, height, width)).to('cuda')  # .cuda()
    model = MUNet(img_size=128, patch_size=1, in_chans=31, out_chans=31,
                  embed_dim=96, depths=[3, 3, 3, 3],
                  mlp_ratio=4.,
                  drop_rate=0., drop_path_rate=0.1,
                  norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                  use_checkpoint=False, final_upsample="Dual up-sample").to('cuda')  # .cuda()
    # print(model)
