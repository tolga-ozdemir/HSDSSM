...
import models.ssumamba.layers as layers
# import layers
...
class MambaLayer(nn.Module):
    def __init__(
        self, dim, d_state=16, d_conv=4, expand=2, channel_token=False, SSAS=None
    ):
        super().__init__()
...
class UMambaEnc(nn.Module):
    ...
    def forward(self, x):
        # === CHANGED: only add channel dim when input is 4D ===
        if x.dim() == 4:  # CHANGED
            # x: [B, T, H, W] -> [B, 1, T, H, W]  (treat T as depth)   # CHANGED
            x = x.unsqueeze(1)  # CHANGED
        elif x.dim() == 5:  # CHANGED
            # x already [B, C, T, H, W]; leave as-is                   # CHANGED
            pass  # CHANGED
        else:  # CHANGED
            raise RuntimeError(  # CHANGED
                f"Expected input with 4 or 5 dims, got {x.dim()} and shape {tuple(x.shape)}"
            )  # CHANGED
        # === END CHANGED ===
