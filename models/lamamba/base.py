...
# 将相对导入改为绝对导入
from models.lamamba.metrics import mpsnr, mse, psnr
...
class BaseModel(pl.LightningModule):
    def __init__(self, optimizer=None, lr_scheduler=None, block_inference=None):
        super().__init__()
    ...
    def forward_blocks(self, x, **kwargs):
        ...
        # x = x.to('cpu')
        # print(x.device)
        block_size = min(max(x.shape[-1], x.shape[-2]), self.block_inference.block_size)
