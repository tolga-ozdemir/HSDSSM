...
from models.lamamba.utils.sync_batchnorm import (
...
class BNReLUDeConv2d(nn.Sequential):
    def __init__(self, in_channels, channels, k=3, s=1, p=1, inplace=False):
        ...
        self.add_module("relu", nn.ReLU(inplace=inplace))
        self.add_module(
            "deconv", nn.ConvTranspose2d(in_channels, channels, k, s, p, bias=False)
        )
...
class BNReLUUpsampleConv2d(nn.Sequential):
    def __init__(
        self, in_channels, channels, k=3, s=1, p=1, upsample=(2, 2), inplace=False
    ):
...
class UpsampleConv2d(torch.nn.Module):
    ...
    def __init__(
        ...
        if upsample:
            self.upsample_layer = torch.nn.Upsample(
                scale_factor=upsample, mode="bilinear", align_corners=True
            )
...
class BasicDeConv2d(nn.Sequential):
    def __init__(self, in_channels, channels, k=3, s=1, p=1, bias=False, bn=True):
        ...
        if bn:
            self.add_module("bn", BatchNorm2d(in_channels))
        self.add_module(
            "deconv", nn.ConvTranspose2d(in_channels, channels, k, s, p, bias=bias)
        )
