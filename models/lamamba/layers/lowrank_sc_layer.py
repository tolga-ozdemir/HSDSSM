...
from models.lamamba.layers.encoding_layer import EncodingLayer
from models.lamamba.layers.soft_thresholding import SoftThresholding
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.patch_centering:
            ones = torch.ones(self.in_channels, 1, self.patch_side, self.patch_side)
            self.ker_mean = (ones / self.patch_side**2).to(device)
        self.ker_divider = torch.ones(1, 1, self.patch_side, self.patch_side).to(device)
        self.divider = None
