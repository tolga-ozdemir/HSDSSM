...
def center_crop_hsi(x, cropx=64, cropy=64):
    """Crop HSI tensor (C, H, W) or (1, C, H, W) to (C, cropy, cropx) centered"""
    if x.ndim == 4:
...
if __name__ == "__main__":
    ...
    mat_transform = Compose([
        LoadMatHSI(input_key='input', gt_key='gt',
                    transform=lambda x: center_crop_hsi(x, 64, 64)),
    ])
    ...
    for i, mat_loader in enumerate(mat_loaders):
        print(f"Loader {i}:")
        for batch in mat_loader:
            print(f"  Input shape: {batch[0].shape}")
            print(f"  GT shape: {batch[1].shape}")
            break
        break
