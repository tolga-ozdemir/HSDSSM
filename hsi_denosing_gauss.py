...
if __name__ == "__main__":
    ...
    train_transform_2 = Compose([AddNoiseBlind([10, 30, 50, 70]), HSI2Tensor()])

    target_transform = HSI2Tensor()

    icvl_64_31_TL_1 = make_dataset(
        opt, train_transform_1, target_transform, common_transform_1, 4
    )

    icvl_64_31_TL_2 = make_dataset(
        opt, train_transform_2, target_transform, common_transform_2, 32
    )
    ...
    mat_datasets = [
        MatDataFromFolder(os.path.join(basefolder, name), size=5) for name in mat_names
    ]
    ...
    mat_datasets = [
        TransformDataset(mat_dataset, mat_transform) for mat_dataset in mat_datasets
    ]
    ...
    while engine.epoch < opt.epochs:
        np.random.seed(
            rand_status_list[engine.epoch % len(rand_status_list)]
        )  # reset seed per epoch, otherwise the noise will be added with a specific pattern
        if engine.epoch == 20:
            adjust_learning_rate(engine.optimizer, base_lr * 0.1)
